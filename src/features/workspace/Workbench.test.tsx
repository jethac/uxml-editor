...
describe('Workbench regions and command bar', () => {
  ...
  it('keeps deterministic canvas tracks stable when diagnostics change', () => {
    ...
    const canvas = screen.getByTestId('canvas-pane');
    const before = {
    ...
    expect(before.width).toBe(String(1920 - 240 - 280 - (WORKBENCH_SEPARATOR_SIZE * 2)));
    expect(before.height).toBe(String(1080 - WORKBENCH_COMMAND_BAR_HEIGHT - 180 - WORKBENCH_SEPARATOR_SIZE));
  });
});
...
describe('PaneResizer behavior', () => {
  it('captures pointer movement, clamps it, and persists only the final dimensions', () => {
    ...
    fireEvent.pointerDown(separator, { pointerId: 7, button: 0, clientX: 240, clientY: 100 });
  ...
  it('removes active pointer listeners when unmounted', () => {
    const store = new EditorStore();
    ...
    fireEvent.pointerDown(separator, { pointerId: 3, button: 0, clientX: 240 });
    unmount();
...
describe.each([
  ...
  it('renders all regions with stable exported track variables', () => {
    ...
    expect(screen.getByTestId('right-pane')).toBeVisible();
    expect(screen.getByTestId('bottom-pane')).toBeVisible();
    expect(screen.getByTestId('canvas-pane')).toHaveAttribute(
      'data-layout-width',
      String(width - 240 - 280 - 8),
    );
  });
});
...
describe('compact 720px layout', () => {
  it('adapts to compact mode when restored side tracks would starve the canvas', () => {
    ...
    expect(screen.getByTestId('canvas-pane')).toHaveAttribute('data-layout-width', '900');
    ...
    expect(Number(screen.getByTestId('canvas-pane').dataset.layoutWidth)).toBeGreaterThanOrEqual(
      WORKBENCH_MIN_CANVAS_WIDTH,
    );
  });
  ...
  it('publishes deterministic non-overlapping command, canvas, and tab-region geometry', () => {
    render(<Workbench store={new EditorStore({ viewport: { width: 720, height: 768 } })} />);
    const commandbar = screen.getByTestId('commandbar');
    const canvas = screen.getByTestId('canvas-pane');
    const tools = screen.getByTestId('compact-tools');
    const commandBottom = Number(commandbar.dataset.layoutTop) + Number(commandbar.dataset.layoutHeight);
    const canvasBottom = Number(canvas.dataset.layoutTop) + Number(canvas.dataset.layoutHeight);

    expect(commandBottom).toBe(Number(canvas.dataset.layoutTop));
    expect(canvasBottom).toBe(Number(tools.dataset.layoutTop));
    expect(Number(canvas.dataset.layoutHeight)).toBe(548);
    expect(Number(tools.dataset.layoutHeight)).toBe(180);
  });
